...
hbb_name = 'boasafra' + ('.exe' if windows else '')
...
def get_features(args):
    ...
    if args.drm:
        ...
        # package NAMED `boasafra` carrying the consent-bypass backend and the root-side uinput
...
def generate_control_file(version):
    ...
    content = """Package: boasafra
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: Boa Safra
Homepage: https://github.com/RafaelPiassi/rustdesk
...
# commit — the same way boasafra sources its other native build deps (vcpkg,
# flutter_rust_bridge, ...), rather than carrying a git submodule. It is the ONLY
# pin for the drm backend: boasafra dlopens this .so at runtime and does not depend on
...
def build_libdrmtap_so():
    ...
    # the shared one explicitly (boasafra dlopens the .so and never needs the archive).
...
DRM_PACKAGE_NAME = 'boasafra-unattended-wayland'
...
def stage_libdrmtap_into_deb(so_path):
    ...
    # symlink is needed: libdrmtap is resolved by ABSOLUTE path (/usr/lib/boasafra/libdrmtap.so.0) at
    # the in-process dlopen site (drmtap_dl.rs), so the deb does NOT drop /usr/lib/boasafra into the
    ...
    system2('mkdir -p tmpdeb/usr/lib/boasafra')
    ...
    system2(f'cp "{so_path}" tmpdeb/usr/lib/boasafra/')
    system2(f'ln -sf "{so_basename}" tmpdeb/usr/lib/boasafra/libdrmtap.so.0')
...
def measured_glibc_floor():
    ...
    paths = [p for p in glob.glob('tmpdeb/usr/lib/boasafra/libdrmtap.so.0.*')
             + glob.glob('tmpdeb/usr/share/boasafra/lib/librustdesk.so')
             + glob.glob('tmpdeb/usr/share/boasafra/boasafra')
...
def retarget_control_to_drm_variant():
    ...
    out = []
    for line in lines:
        if line.startswith('Package: boasafra'):
            out.append(f'Package: {DRM_PACKAGE_NAME}\n')
            out.append('Conflicts: boasafra\nReplaces: boasafra\nProvides: boasafra\n')
            ...
            out.append(line)
    body = ''.join(out)
    # Fail loudly rather than silently shipping a package that says `boasafra`: a stock control file
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/boasafra')
    system2('mkdir -p tmpdeb/usr/share/boasafra/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/boasafra || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/boasafra/')
    system2(
        'cp ../res/boasafra.service tmpdeb/usr/share/boasafra/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/boasafra.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/boasafra.svg')
    system2(
        'cp ../res/boasafra.desktop tmpdeb/usr/share/applications/boasafra.desktop')
    system2(
        'cp ../res/boasafra-link.desktop tmpdeb/usr/share/applications/boasafra-link.desktop')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/boasafra/files/polkit && chmod a+x tmpdeb/usr/share/boasafra/files/polkit")
    ...
    system2('dpkg-deb -b tmpdeb boasafra.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('boasafra.deb', '../boasafra-%s.deb' % version)
    if ships_so:
        # Named apart from the stock package so installing the consent-free variant is a deliberate act.
        os.rename('../boasafra-%s.deb' % version, f'../{DRM_PACKAGE_NAME}-{version}.deb')
    os.chdir("..")


DRMTAP_DLOPEN_MARKER = b'/usr/lib/boasafra/libdrmtap.so.0'
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/boasafra/lib/librustdesk.so')
                + glob.glob('tmpdeb/usr/share/boasafra/boasafra') if os.path.isfile(p)]
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    ...
    system2('rm tmpdeb/usr/bin/boasafra || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/boasafra/')
    system2(
        'cp ../res/boasafra.service tmpdeb/usr/share/boasafra/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/boasafra.png')
    ...
    bundled_glob = glob.glob('tmpdeb/usr/share/boasafra/libdrmtap.so.0.*')
    ...
    if want_drm:
        ...
        if bundle_carries_so:
            ...
            system2(f'rm -f "{so}"')
            system2('rm -f tmpdeb/usr/share/boasafra/libdrmtap.so tmpdeb/usr/share/boasafra/libdrmtap.so.0')
        else:
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/BoaSafra.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"BoaSafra Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon BoaSafra.app 200 190 --hide-extension BoaSafra.app boasafra.dmg ./build/macos/Build/Products/Release/BoaSafra.app")
    os.rename("boasafra.dmg", f"../boasafra-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/boasafra.exe')
    os.chdir('../..')
    if os.path.exists('./boasafra_portable.exe'):
        os.replace('./target/release/rustdesk-portable-packer.exe',
                   './boasafra_portable.exe')
    else:
        os.rename('./target/release/rustdesk-portable-packer.exe',
                  './boasafra_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/boasafra_portable.exe')
    os.rename('./boasafra_portable.exe', f'./boasafra-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/boasafra-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/boasafra.exe')
        system2('mv target/release/boasafra.exe target/release/BoaSafra.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\boasafra.exe')
        else:
            print('Not signed')
        os.makedirs(res_dir, exist_ok=True)
        system2(
            f'cp -rf target/release/BoaSafra.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/boasafra-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/boasafra-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --locked --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/boasafra')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv boasafra-%s-0-x86_64.pkg.tar.zst boasafra-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./boasafra.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/boasafra-%s-0.x86_64.rpm ./boasafra-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall boasafra.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/boasafra-%s-0.x86_64.rpm ./boasafra-%s-suse.rpm' % (
                version, version))
        # yum localinstall boasafra.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/boasafra*.deb ./flutter/boasafra.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo --locked bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/BoaSafra.app/Contents/MacOS/boasafra')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/BoaSafra.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/boasafra-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/BoaSafra.app/Contents/MacOS/boasafra
    #rcodesign sign --p12-file ~/.p12/boasafra-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/BoaSafra.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/boasafra-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/BoaSafra.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/BoaSafra.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/BoaSafra.app
    '''.format(pa))
                system2(
                    'create-dmg "BoaSafra %s.dmg" "target/release/bundle/osx/BoaSafra.app"' % version)
                os.rename('BoaSafra %s.dmg' %
                          version, 'boasafra-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/boasafra-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./boasafra-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./boasafra-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple boasafra-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/BoaSafra.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/boasafra*.deb ./boasafra.deb')
                system2('dpkg-deb -R boasafra.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/boasafra/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2('strip tmpdeb/usr/bin/boasafra')
                system2('mkdir -p tmpdeb/usr/share/boasafra')
                system2('mv tmpdeb/usr/bin/boasafra tmpdeb/usr/share/boasafra/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/boasafra/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb boasafra.deb; /bin/rm -rf tmpdeb/')
                os.rename('boasafra.deb', 'boasafra-%s.deb' % version)
